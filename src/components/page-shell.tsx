...
export function PageShell({ title, description, children, action, breadcrumb, hideTitle }: { title: string; description?: string; children: React.ReactNode; action?: React.ReactNode; breadcrumb?: { label: string; href?: string }[]; hideTitle?: boolean }) {
  return (
    <div className="mx-auto w-full max-w-[1600px] space-y-4 pb-6 sm:space-y-6 sm:pb-8">
      {breadcrumb && <Breadcrumb items={breadcrumb} />}
      {!hideTitle && <div className="flex flex-col gap-4 rounded-lg border bg-card/90 p-4 shadow-sm sm:flex-row sm:items-start sm:justify-between sm:p-5">
        <div>
