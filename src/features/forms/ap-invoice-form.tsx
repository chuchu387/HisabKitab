...
import { createApInvoice } from "@/actions/ap-invoices";
...
export function ApInvoiceForm({ projects, purchaseOrders }: { projects: any[]; purchaseOrders: any[] }) {
  const [state, formAction, pending] = useActionState(createApInvoice, initialState);
  const today = new Date().toISOString().slice(0, 10);
  return (
    <form action={formAction} className="grid gap-4 rounded-lg border bg-card p-4 shadow-sm md:grid-cols-3">
      <Field name="billNumber" label="Bill No." placeholder="Supplier invoice no." />
      ...
      <div className="space-y-2">
        ...
        <Select name="purchaseOrderId" defaultValue="">
          ...
          {purchaseOrders.map((order) => <option key={order._id} value={order._id}>{order.orderNumber} - {order.vendorName}</option>)}
        </Select>
      </div>
      <div className="space-y-2">
        <Label>Project</Label>
      ...
      <div className="space-y-2">
        <Label>Status</Label>
        <Select name="status" defaultValue="posted">
      ...
      <div className="md:col-span-3"><Field name="description" label="Line Description" placeholder="Vendor bill details" /></div>
      ...
      <div className="grid gap-3 md:col-span-3">
        ...
        <Button disabled={pending}>{pending ? "Creating..." : "Create AP Invoice"}</Button>
      </div>
    </form>
  );
}
